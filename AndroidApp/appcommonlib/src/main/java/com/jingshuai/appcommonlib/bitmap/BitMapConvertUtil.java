...
public class BitMapConvertUtil {
    ...
    public static Bitmap getSmallBitmap(String filePath, int w, int h) {
        ...
        Bitmap bitmap = BitmapFactory.decodeFile(filePath, options);// BitmapFactory.decodeFile()????????????
        return bitmap;
    }
    ...
    public static void setImageByResourseId(Context context, ImageView imageView, int resourseId) {
        if (imageView == null)
            return;
        Bitmap bitmap = BitMapConvertUtil.getBitmapFromResource(context, resourseId);
        if (bitmap == null)
            return;
